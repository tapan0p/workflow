<script type="text/javascript">
    ...
    RED.nodes.registerType('triplets',{
        ...
        onpaletteadd: function() {
            // General message handler for all triplet nodes
            function handleTripletMessage(topic, data) {
                console.log("Received triplets_history message:", data);
                if (data && data.id && data.msg) {
                    var nodeId = data.id;
                    
                    // Store the received data in our handlers object
                    if (!tripletsHandlers[nodeId]) {
                        tripletsHandlers[nodeId] = { data: [] };
                    }
                    
                    // Update stored data
                    tripletsHandlers[nodeId].data = data.msg;
                    ...
                    // Log for debugging
                    console.log("Received triplets_history for node:", nodeId, data.msg);
                }
            }
            
            // Subscribe to all triplets history messages
            RED.comms.subscribe("triplets_history/+", handleTripletMessage);
            
            // Clean up on palette removal
            this.onpaletteremove = function() {
                RED.comms.unsubscribe("triplets_history/+", handleTripletMessage);
            };
        },
        oneditprepare: function() {
            ...
            if (tripletsHandlers[this.id] && tripletsHandlers[this.id].data) {
                var data = tripletsHandlers[this.id].data;
                outputPreviewContainer.empty();
            ...
            $(document).on('doxigen-data-loaded', function(event, data) {
                console.log("Received doxigen-data-loaded event with data:", data);
                if (data && data.tools && data.tools.length > 0) {
                    window.doxigenData = data;
                    window.toolsList = data.tools;
                    
                
                    
                    // Clear existing editors
                    for (var id in functionEditors) {
                        if (functionEditors.hasOwnProperty(id)) {
                            functionEditors[id].destroy();
                        }
                    }
                    ...
                    // Add the tools from doxigen_data.json
                    for (var i=0; i<data.tools.length; i++) {
                        addFunctionRow(data.tools[i], functionCounter++);
                    }
                    
                    // Then add any custom functions saved with the node that aren't in the predefined list
                    if (node.functions && node.functions.length) {
                        var existingNames = data.tools.map(function(tool) { return tool.name; });
                        
                        for (var i=0; i<node.functions.length; i++) {
                            // Only add functions that aren't already in the predefined tools
                            if (!existingNames.includes(node.functions[i].name)) {
                    ...
                    node.functionEditors = functionEditors;
                }
            });
            
            $(document).on('triplets-data-loaded', function(event, data) {
                console.log("Received triplets-data-loaded event with data:", data);
                if (data) {
                    window.tripletsData = data;
                    window.tripletEntities = data.entities || [];
                    window.tripletRelations = data.relations || [];
                    
                    // Sort entities and relations alphabetically
                    if (window.tripletEntities && window.tripletEntities.length) {
                        window.tripletEntities.sort();
                        console.log("Sorted entities from event:", window.tripletEntities);
                    }
                    
                    if (window.tripletRelations && window.tripletRelations.length) {
                        window.tripletRelations.sort();
                        console.log("Sorted relations from event:", window.tripletRelations);
                    }
                    
                    // Update all existing subject dropdowns
                    $(".node-input-template-subject").each(function() {
                        ...
                        select.empty();
                        select.append($('<option value="">-- Subject --</option>'));
                    ...
                    // Update all existing predicate dropdowns
                    $(".node-input-template-predicate").each(function() {
                        var select = $(this);
                        var currentValue = select.val();
                        
                        // Clear and re-add the default option
                        select.empty();
                        select.append($('<option value="">-- Predicate --</option>'));
            ...
            console.log("Checking for triplet entities in window.tripletsData:", window.tripletsData);
            window.tripletEntities = window.tripletsData ? window.tripletsData.entities || [] : [];
            window.tripletRelations = window.tripletsData ? window.tripletsData.relations || [] : [];
            
            // Sort the entities and relations alphabetically
            if (window.tripletEntities && window.tripletEntities.length) {
                window.tripletEntities.sort();
                console.log("Sorted entities:", window.tripletEntities);
            }
            
            if (window.tripletRelations && window.tripletRelations.length) {
                window.tripletRelations.sort();
                console.log("Sorted relations:", window.tripletRelations);
            }
            
            // Extract tools from dictionary
            console.log("Checking for tools in window.doxigenData:", window.doxigenData);
            window.toolsList = window.doxigenData ? window.doxigenData.tools || [] : [];
            console.log("Found tools:", window.toolsList);
            
            // If doxigen data is not loaded yet, fetch it explicitly
            if (!window.doxigenData || !window.doxigenData.tools || window.doxigenData.tools.length === 0) {
                console.log("Doxigen data not loaded yet, fetching it explicitly...");
                $.ajax({
                    url: "/api/load_doxigen?t=" + new Date().getTime(),
                    method: "GET",
                    cache: false,
                    success: function(data) {
                        console.log("Explicitly fetched doxigen data:", data);
                        window.doxigenData = data;
                        window.toolsList = data.tools || [];
                        
                        // Trigger the same loading logic as the event handler
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        console.error("Error explicitly fetching doxigen data:", textStatus, errorThrown);
                    }
                });
            }
            ...
            function initializeTemplates() {
                ...
                if (node.templates && node.templates.length) {
                    ...
                    addTemplateRow({subject:"Patient", predicate:"has_appointment_with", object:"", enableStorage: false});
                    addTemplateRow({subject:"Patient", predicate:"has_symptom", object:"", enableStorage: false});
                }
            }
            ...
            function addTemplateRow(template) {
                ...
                if (window.tripletEntities && window.tripletEntities.length) {
                    ...
                    var sortedEntities = window.tripletEntities.slice().sort();
                ...
                if (window.tripletRelations && window.tripletRelations.length) {
                    // Ensure relations are sorted
                    var sortedRelations = window.tripletRelations.slice().sort();
                    sortedRelations.forEach(function(relation) {
                        var option = $('<option/>').text(relation).val(relation);
            ...
            // Load tools
            if (window.toolsList && window.toolsList.length) {
                // Always use predefined tools from doxigen_data.json first
                console.log("Loading predefined tools from toolsList:", window.toolsList.length);
                ...
                // Then add any custom functions saved with the node that aren't in the predefined list
                if (node.functions && node.functions.length) {
                    console.log("Adding node's custom saved functions:", node.functions.length);
                    var existingNames = window.toolsList.map(function(tool) { return tool.name; });
                ...
                console.log("No predefined tools available, using node's saved functions:", node.functions.length);
                for (var i=0; i<node.functions.length; i++) {
                    addFunctionRow(node.functions[i], functionCounter++);
                }
            } else {
                console.log("No tools available, adding default tool");
        ...
        oneditsave: function() {
            ...
            // When saving, only save custom functions that aren't from the doxigen data file
            var functions = [];
            var doxigenToolNames = window.doxigenData && window.doxigenData.tools ? 
                window.doxigenData.tools.map(function(tool) { return tool.name; }) : [];
            ...
            $(".function-row").each(function() {
                ...
                // If this is a custom function (not from doxigen) or it's been modified, save it
                var isDoxigenTool = doxigenToolNames.includes(name);
                var toolIndex = isDoxigenTool ? doxigenToolNames.indexOf(name) : -1;
                var originalCode = toolIndex >= 0 ? window.doxigenData.tools[toolIndex].code : "";
                var isModified = code !== originalCode;
                
                if (!isDoxigenTool || isModified) {
                    functions.push({
                        name: name,
                        code: code
                    });
            ...
            // If we have no custom functions, store an empty array to ensure doxigen tools are loaded next time
        ...
        _cleanUpComms: function() {
            ...
            if (this._outputMessageHandler) {
                RED.comms.unsubscribe("triplets/" + this.id, this._outputMessageHandler);
...
<script type="text/html" data-template-name = "triplets">
    ...
    <div id="triplets-editor-container" style="display: flex; width: 100%; height: 100%;">

        <!-- Left Column (70%): Existing Properties -->
        <div id="triplets-properties-column" style="flex: 0 0 70%; padding-right: 15px; box-sizing: border-box; overflow-y: auto;">
        ...
        <div id="triplets-output-column" style="flex: 1; padding-left: 5px; border-left: 1px solid #ccc;">
