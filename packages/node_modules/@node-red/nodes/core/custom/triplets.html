<script type="text/javascript">
    ...
    RED.nodes.registerType('triplets',{
        ...
        oneditprepare: function() {
            ...
            window.tripletEntities = window.tripletsData.entities;
            window.tripletRelations = window.tripletsData.relations;
            
            // Extract tools from dictionary
            window.toolsList = window.doxigenData.tools;
            ...
            // Add placeholder to output preview
            outputPreviewContainer.html("<p><i>Loading message history...</i></p>");

            // Create a local array to store message history
            var nodeOutputs = []; 

            // Function to update the preview display
            function updatePreview() {
                console.log("Updating preview with", nodeOutputs.length, "messages");
                ...
                if (nodeOutputs.length === 0) {
                    outputPreviewContainer.html("<p><i>No message history available</i></p>");
                    return;
                }
            ...
            // Create a message handler for real-time updates
            var messageHandler = function(topic, msg) {
                try {
                    console.log("Received message via comms:", topic);
                    
                    // Convert message to string for display
                    var outputStr = JSON.stringify(msg, null, 2);
                    
                    // Add to front of array (newest first)
                    nodeOutputs.unshift(outputStr);
                    
                    // Update the display with all messages
                    updatePreview();
                } catch(e) {
                    console.error("Error processing message for preview:", e);
                }
            };

            // Subscribe to the node's output topic for real-time updates
            var outputTopic = "triplets/output/" + this.id;
            console.log("Subscribing to topic:", outputTopic);
            RED.comms.subscribe(outputTopic, messageHandler);

            // Store handler reference for cleanup
            this._outputMessageHandler = messageHandler;

            // Load history from file when the page loads or refreshes
            console.log("Attempting to load message history...");
            $.ajax({
                url: "data/message_history.json",
                dataType: "json",
                cache: false,
                success: function(history) {
                    console.log("Successfully loaded message history file");
                    try {
                        // Find messages for this node
                        var nodeHistory = history && history.node_messages && history.node_messages[node.id] || [];
                        console.log("Found", nodeHistory.length, "messages for node", node.id);
                        
                        // Convert to string format for display
                        nodeOutputs = nodeHistory.map(function(entry) {
                        ...
                        console.error("Error processing message history:", e);
                        outputPreviewContainer.html("<p><i>Error loading message history: " + e.message + "</i></p>");
                    }
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    console.log("Error loading message history:", textStatus, errorThrown);
                    outputPreviewContainer.html("<p><i>No message history available or error loading it</i></p>");
                }
            });
        },
        oneditsave: function() {
        ...
        _cleanUpComms: function() {
            ...
            if (this._outputMessageHandler) {
                RED.comms.unsubscribe("triplets/output/" + this.id, this._outputMessageHandler);
...
<script type="text/html" data-template-name = "triplets">
    ...
    <div id="triplets-editor-container" style="display: flex; width: 100%; height: 100%;">
        ...
        <div id="triplets-output-column" style="flex: 1; display: flex; flex-direction: column; padding-left: 15px; box-sizing: border-box; border-left: 1px solid #ccc; height: 100%;">
            <h4 style="margin-top: 0; margin-bottom: 10px; border-bottom: 1px solid #eee; padding-bottom: 5px; flex-shrink: 0;">Node Output Preview</h4>
            <!-- Added flex-grow: 1 to make preview area fill space -->
            <div id="node-output-preview" style="flex-grow: 1; overflow-y: auto; border: 1px solid #eee; padding: 10px; background-color: #f9f9f9; font-family: monospace; font-size: 12px; white-space: pre-wrap; word-wrap: break-word;">
