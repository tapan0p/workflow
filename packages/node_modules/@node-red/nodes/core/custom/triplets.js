module.exports = function(RED) {
    ...
    function Triplets(config) {
        ...
        var node = this;
        node.outputHistory = []; // Store history locally (as stringified JSON)
        ...
        function sendAndUpdatePreview(msg) {
            node.log("sendAndUpdatePreview called.");
            if (!msg) {
                 node.log("Message is null/undefined, skipping send.");
                 return;
            }

            try {
                let previewMsg = JSON.stringify(msg, null, 2);

                node.outputHistory.unshift(previewMsg);
                if (node.outputHistory.length > 50) {
                    node.outputHistory.pop();
                }

                const topic = "triplets/output/" + node.id;
                node.log(`Publishing to comms topic: ${topic}`);
                RED.comms.publish(topic, msg, true);

                node.log("Sending message to downstream nodes.");
                node.send(msg);

            } catch (e) {
                 node.warn("Could not stringify message for preview history: " + e.toString());
                 try {
                     const topic = "triplets/output/" + node.id;
                     node.log(`Publishing raw message to comms topic (stringify failed): ${topic}`);
                     RED.comms.publish(topic, msg, true);
                     node.send(msg); // Send the original msg
                 } catch (e2) {
                     node.error("Error sending output for preview after stringify failed: " + e2.toString(), msg);
                     node.send(msg); // Still attempt to send the original message
                 }
            }
        }
        ...
        // Add endpoint for editor to fetch initial history
        RED.httpAdmin.get("/triplets/" + node.id + "/outputHistory", RED.auth.needsPermission("triplets.read"), function(req, res) {
            var targetNode = RED.nodes.getNode(req.params.id);
            node.log(`History endpoint called for node: ${req.params.id}`);
            if (targetNode != null) {
                try {
                    node.log(`Returning history for node ${targetNode.id}, length: ${(targetNode.outputHistory || []).length}`);
                    res.json(targetNode.outputHistory || []);
                } catch(err) {
                    node.error(`Error in history endpoint for node ${targetNode.id}: ${err.toString()}`);
                    res.sendStatus(500);
                    targetNode.error(RED._("httpadmin.errors.error-loading-node-config",{message:err.toString()}));
                }
            } else {
                node.warn(`History endpoint called for non-existent node: ${req.params.id}`);
                res.sendStatus(404);
            }
        });
    }
