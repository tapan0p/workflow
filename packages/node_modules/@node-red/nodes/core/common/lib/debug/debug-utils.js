...
RED.debug = (function() {
    ...
    function handleDebugMessage(o) {
        ...
        if (!busy && (stack.length > 0)) {
            busy = true;
            // Check if we should scroll to top (for new messages)
            // Measure from top instead of bottom since we reversed the display order
            var atTop = (sbc.scrollTop <= 5);
            processDebugMessage(stack.shift(), null, atTop);
            setTimeout(function() {
                busy = false;
                handleDebugMessage();
    ...
    function processDebugMessage(o, key, atTop) {
        var msg = $("<div/>");
