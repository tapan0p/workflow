    ...
    hooks.trigger("preRoute",sendEvent,(err) => {
        if (err) {
            ...
            if (sendEvent.destination.node && typeof sendEvent.destination.node === 'object') {
                // Check if this message should be skipped based on link conditions
                const sourceNode = sendEvent.source.node;
                const targetId = sendEvent.destination.id;
                const portIndex = sendEvent.source.port || 0;
                
                // If source node has _linkConditions property, check if this message should pass
                if (sourceNode && sourceNode._linkConditions && 
                    sourceNode._linkConditions[portIndex] && 
                    sourceNode._linkConditions[portIndex][targetId]) {
                    
                    // Get the condition
                    const condition = sourceNode._linkConditions[portIndex][targetId];
                    
                    // Evaluate the condition (if the source node has the evaluateCondition method)
                    if (condition && typeof sourceNode.evaluateCondition === 'function') {
                        try {
                            // If condition evaluates to false, skip routing this message
                            if (!sourceNode.evaluateCondition(condition, sendEvent.msg)) {
                            ...
                            flow.log({
                                ...
                                msg: "Error evaluating link condition: " + err.toString()
                            });
                        }
                    }
                }
