...
async function createNode(flow,config) {
    ...
    try {
        ...
        if (typeof nodeTypeConstructor === "function") {
            ...
            // --- Process wires before passing to constructor ---
            if (conf.wires && Array.isArray(conf.wires)) {
                conf._linkConditions = {}; // Store conditions here
                const originalWires = conf.wires; // Keep original structure if needed elsewhere
                const simpleWires = [];
                originalWires.forEach((wireGroup, i) => {
                    if (Array.isArray(wireGroup)) {
                        const simpleWireGroup = [];
                        wireGroup.forEach(wire => {
                            if (wire && typeof wire === 'object' && wire.target) {
                                simpleWireGroup.push(wire.target); // Use only target for constructor
                                if (wire.conditions) {
                                    // Store conditions keyed by target ID for this node
                                    if (!conf._linkConditions[i]) { conf._linkConditions[i] = {}; }
                                    conf._linkConditions[i][wire.target] = wire.conditions;
                                }
                            } else if (typeof wire === 'string') {
                                simpleWireGroup.push(wire); // Keep old format as is
                            }
                        });
                         simpleWires.push(simpleWireGroup);
                    } else {
                        simpleWires.push(wireGroup); // Keep invalid/empty groups as is
                    }
                });
                conf.wires = simpleWires; // Pass simplified wires to constructor
                // Optionally keep original wires if needed later: conf._originalWiresWithConditions = originalWires;
            }
            // --- End wire processing ---
...
function validateFlow(flow) {
    ...
    if (!Array.isArray(flow)) {
        if (typeof flow === 'object' && flow.flows) {
            // Handle flow format from editor
            flow = flow.flows;
        } else {
            // Attempt to handle cases where flow might be an object with numeric keys
            try {
                flow = Object.values(flow);
                if (!flow.every(item => typeof item === 'object' && item.id)) {
                     throw new Error("Invalid flow format - expected array or standard flow object");
                }
            } catch (e) {
                 throw new Error("Invalid flow format - expected array or standard flow object");
            }
    ...
    flow.forEach(node => {
        // Validate nodes, configs, etc.
        if (!node.id || !node.type) {
            throw new Error(`Invalid node structure: missing id or type: ${JSON.stringify(node)}`);
        }

        // Validate wires format
        if (node.wires) {
            if (!Array.isArray(node.wires)) {
                throw new Error(`Node ${node.id} has invalid 'wires' format (not an array).`);
            }
            
            node.wires.forEach((wireGroup, portIndex) => {
                if (!Array.isArray(wireGroup)) {
                    throw new Error(`Node ${node.id}, port ${portIndex} has invalid wire group (not an array).`);
                }
                
                wireGroup.forEach((wire, wireIndex) => {
                    const isValidOldFormat = typeof wire === 'string';
                    const isValidNewFormat = wire && typeof wire === 'object' && typeof wire.target === 'string';

                    if (!isValidOldFormat && !isValidNewFormat) {
                        throw new Error(`Node ${node.id}, port ${portIndex}, wire ${wireIndex} has invalid format: ${JSON.stringify(wire)}`);
                    }
                    
                    // Validate conditions if present
                    if (isValidNewFormat && wire.conditions) {
                        // Check logical conditions
                        if (wire.conditions.logical) {
                            if (!wire.conditions.logical.conditions || !Array.isArray(wire.conditions.logical.conditions)) {
                                throw new Error(`Node ${node.id}, port ${portIndex} has invalid logical conditions format.`);
                            }
                        }
                        
                        // Check prompt conditions
                        if (wire.conditions.prompt) {
                            if (!wire.conditions.prompt.conditions || !Array.isArray(wire.conditions.prompt.conditions)) {
                                throw new Error(`Node ${node.id}, port ${portIndex} has invalid prompt conditions format.`);
                            }
                        }
                    }
                });
...
module.exports = {
    ...
    validateFlow,
    // Remove prepareFlowForExport if it was added here erroneously
    // prepareFlowForExport // This function belongs in the editor client (actions.js)
}
