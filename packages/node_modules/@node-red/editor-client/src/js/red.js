var RED = (function() {
        ...
        $.ajax({
            ...
            success: function(nodes) {
                if (nodes) {
                    ...
                    try {
                        // Calls the main import function to reconstruct all nodes, wires, tabs, etc. in the editor.
                        RED.nodes.import(nodes.flows);
                        //Marks the editor as "clean" (no unsaved changes).
                        RED.nodes.dirty(false);
                        // Forces a redraw of the editor.
    ...
    function buildMainMenu() {
        ...
        menuOptions.push({
            id: "menu-item-mqtt-config",
            label: "Global MQTT Settings",
            onselect: function() {
                RED.tray.show({
                    ...
                    buttons: [
                        ...
                        },
                        {
                            ...
                            class: "primary",
                            click: function() {
                                var broker = $("#dialog-form-mqtt-broker").val();
                                var port = $("#dialog-form-mqtt-port").val();
                                var qos = $("#dialog-form-mqtt-qos").val();

                                // Basic validation
                                if (!broker || !port) {
                                    RED.notify("Broker and Port are required.", "warning");
                                    return;
                                }
                                ...
                                // Restore AJAX call to save settings
                                $.ajax({
                                    ...
                                    success: function(response) {
                                        console.log("(UI) Save successful, server response:", response);
                                        if (response && response.status === "success") {
                                            RED.notify("MQTT Broker Configuration saved successfully", "success");
                                            RED.tray.close();
                                        } else {
                                            RED.notify("Error saving MQTT configuration: Unexpected response format", "error");
                                        }
                                    },
                                    error: function(jqXHR, textStatus, errorThrown) {
                                        console.error("(UI) Error saving MQTT settings:", {
                                            status: jqXHR.status,
                                        ...
                                        var errorMsg = "Error saving MQTT Configuration";
                                        try {
                                            var response = JSON.parse(jqXHR.responseText);
                                            errorMsg += ": " + (response.message || response.error || errorThrown);
                                        } catch(e) {
                    ...
                    open: function(tray) {
                        console.log("Opening Global MQTT Settings tray");
                        var trayBody = tray.find('.red-ui-tray-body');
                        var form = $('<form class="node-dialog-edit-form form-horizontal"></form>');

                        // Broker Row
                        var brokerRow = $('<div class="form-row"></div>').appendTo(form);
                        $('<label for="dialog-form-mqtt-broker"><i class="fa fa-server"></i> Broker</label>').appendTo(brokerRow);
                        var brokerInput = $('<input type="text" id="dialog-form-mqtt-broker" placeholder="e.g., mqtt.example.com or 192.168.1.100">'
                        ).appendTo(brokerRow);

                        // Port Row
                        var portRow = $('<div class="form-row"></div>').appendTo(form);
                        $('<label for="dialog-form-mqtt-port"><i class="fa fa-plug"></i> Port</label>').appendTo(portRow);
                        var portInput = $('<input type="text" id="dialog-form-mqtt-port" placeholder="e.g., 1883 or 8883">'
                        ).appendTo(portRow);

                        // QoS Row
                        var qosRow = $('<div class="form-row"></div>').appendTo(form);
                        $('<label for="dialog-form-mqtt-qos"><i class="fa fa-sliders"></i> QoS</label>').appendTo(qosRow);
                        var qosSelect = $('<select id="dialog-form-mqtt-qos"></select>').appendTo(qosRow);
                        qosSelect.append($('<option value="0">0 - At most once</option>'));
                        qosSelect.append($('<option value="1">1 - At least once</option>'));
                        qosSelect.append($('<option value="2">2 - Exactly once</option>'));

                        form.appendTo(trayBody);

                        // Restore AJAX call to load settings
                        console.log("(UI) Loading current MQTT settings via GET /settings/global-mqtt");
                        $.getJSON("settings/global-mqtt")
                            .done(function(data) {
                                console.log("(UI) Loaded MQTT settings:", data);
                                brokerInput.val(data.broker || "");
                                portInput.val(data.port || "");
                                qosSelect.val(data.qos || "0");
                            })
                            .fail(function(jqXHR, textStatus, errorThrown) {
                                console.error("(UI) Error loading MQTT settings:", textStatus, errorThrown);
                                RED.notify("Failed to load current MQTT settings.", "error");
                                // Set defaults manually if load fails
                                brokerInput.val("localhost");
                                portInput.val("1883");
                                qosSelect.val("0");
                            });
                    },
                    close: function() {
                        console.log("Closing Global MQTT Settings tray");
                    }
                });
                return false;
            }
