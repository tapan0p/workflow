...
var RED = (function() {
    ...
    function buildMainMenu() {
        ...
        menuOptions.push({
            ...
            onselect: function() {
                var dialog = $('<div id="mqtt-config-dialog" class="hide node-red-dialog"></div>')
                    .appendTo("body")
                    .dialog({
                        modal: true,
                        autoOpen: true,
                        ...
                        buttons: [
                            {
                                ...
                                class: "primary",
                                click: function() {
                                    var broker = $("#node-config-input-broker").val();
                                    ...
                                    $.ajax({
                                        ...
                                        success: function(response) {
                                            ...
                                            if (response && response.status === "success") {
                                                RED.settings.globalMqttConfig = response.config;
                                                RED.notify("MQTT Broker Configuration saved", "success");
                                                dialog.dialog("close");
                                            } else {
                                                RED.notify("Unexpected response format", "error");
                                            }
                                        },
                                        error: function(jqXHR, textStatus, errorThrown) {
                                            console.error("Error saving MQTT settings:", {
                                                status: jqXHR.status,
                                            ...
                                            RED.notify("Error saving MQTT Configuration: " + errorMsg, "error");
                                        }
                                    });
                                }
                            },
                        ...
                        open: function() {
                            console.log("Loading current MQTT settings");
                            // Get current settings from the server
                            $.getJSON("settings/global-mqtt", function(data) {
                                console.log("Loaded MQTT settings:", data);
                                $("#node-config-input-broker").val(data.broker || "localhost");
                                $("#node-config-input-port").val(data.port || "1883");
                            }).fail(function(jqXHR, textStatus, errorThrown) {
                                console.error("Error loading MQTT settings:", textStatus, errorThrown);
                                // Use default values if loading fails
                                $("#node-config-input-broker").val("localhost");
                ...
                var dialogForm = $('<form class="form-horizontal"></form>').appendTo(dialog);

                $('<div class="form-row">' +
                    '<label for="node-config-input-broker">MQTT Broker</label>' +
