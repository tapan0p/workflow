RED.contextMenu = (function () {
    ...
    RED.editConnection = function(link) {
        ...
        // --- MODIFIED: Prioritize link._condition --- 
        var conditionData = null;
        if (link._condition) {
            // Use the condition directly attached to the link object (from import)
            console.log("[RED.editConnection] Found condition on link object for:", linkId);
            conditionData = link._condition;
        } else if (edgeConditions[linkId]) {
            // Fallback to checking the global map
            console.log("[RED.editConnection] Found condition in global map for:", linkId);
            conditionData = edgeConditions[linkId];
        } else {
            // No condition found, initialize empty
            console.log("[RED.editConnection] No condition found for:", linkId, "Initializing empty.");
            conditionData = {};
        }

        // Use deep copy to avoid modifying original during edit
        var conditionCopy = JSON.parse(JSON.stringify(conditionData)); 
