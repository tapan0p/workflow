RED.contextMenu = (function () {
    ...
    function showConnectionDialog(link, condition) {
        ...
        if (!condition.logical) {
            condition.logical = { logic: 'and', conditions: [] };
        }
        if (!condition.prompt) {
            condition.prompt = { conditions: [] };
        }

        // Migrate old format if necessary
        if (!condition.logical.conditions || condition.logical.conditions.length === 0) {
            if (Array.isArray(condition.conditions)) {
                 condition.logical.conditions = condition.conditions;
                 condition.logical.logic = condition.logic || 'and';
                ...
                condition.logical.conditions.push({
                ...
                delete condition.tripletSubject;
                delete condition.tripletPredicate;
                delete condition.tripletObject;
            }
        }
        ...
        // --- Save Function (Modified for selects) ---
            ...
            $('#prompt-conditions-list .prompt-condition-row').each(function() {
                var row = $(this);
                var promptText = row.find('.condition-prompt-input').data('full-prompt')?.trim();
                if (promptText) {
