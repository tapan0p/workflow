RED.deploy = (function() {
    ...
    function save(skipValidation, force) {
        ...
        // Original: const data = { flows: nns };
        // NEW: Create a flat array from the 'nns' object which comes from createCompleteNodeSet
        const nodeSet = nns; // Assuming nns is the result of createCompleteNodeSet
        const flatFlowPayload = [
            ...(nodeSet.workspaces || []),
            ...(nodeSet.subflows || []),
            ...(nodeSet.configNodes || []),
            ...(nodeSet.nodes || []),
        ...
        console.log("[Deploy] Sending payload:", JSON.stringify(flatFlowPayload, null, 2));
        
        const data = { flows: flatFlowPayload }; // Keep the outer structure { flows: ... } for now if needed by the server endpoint wrapper, but the inner value is now flat
        if (!force) {
        ...
        // Save edge conditions to settings before deploy
        if (RED.contextMenu && typeof RED.contextMenu.getAllConditions === 'function') {
            try {
                const allEdgeConditions = RED.contextMenu.getAllConditions();
                RED.settings.set('edgeConditions', allEdgeConditions || {});
                // console.log("Saving edge conditions to settings:", allEdgeConditions);
            } catch (err) {
                console.error("Error saving edge conditions:", err);
                // Decide if deployment should be halted? For now, proceed.
            }
        } else {
             // Ensure setting is cleared if context menu is not available
             RED.settings.set('edgeConditions', {});
        }

        $("#red-ui-header-button-deploy-options").removeClass("open");
        $.ajax({
            url: "flows",
            type: "POST",
            data: JSON.stringify(data), // Send the data object which now contains the flat array
