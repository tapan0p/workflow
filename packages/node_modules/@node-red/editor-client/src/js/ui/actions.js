RED.actions = (function() {
    ...
    function prepareFlowForExport(flow) {
        return flow.map(node => {
            const newNode = {...node};
            
            // Convert wires to simple format for export
            if (newNode.wires) {
                newNode.wires = newNode.wires.map(wireGroup => 
                    wireGroup.map(wire => 
                        typeof wire === 'object' ? wire.target : wire
                    )
                );
            }
