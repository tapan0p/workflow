RED.palette = (function() {
    ...
    function createCategory(originalCategory,rootCategory,category,ns) {
        // Do nothing - no categories needed
    }

    function createCategoryContainer(originalCategory,category, labelId) {
        // Do nothing - we're using a flat structure
        categoryContainers[category] = {
            container: $("#red-ui-palette-container"),
            hide: function () {},
            show: function () {},
            isOpen: function () { return true; },
            getNodeCount: function () { return 0; },
            close: function() {},
            open: function() {},
            toggle: function() {}
        };
    ...
    // Implement a helper function to check if a node should be filtered
    function shouldFilterNode(nodeType, nodeName, def) {
        const simpleName = def.paletteLabel || def.label || nodeName;
        console.log(`Checking node: ${nodeType}, simpleName: ${simpleName}`);
        
        // Try different matching strategies
        
        // 1. Direct full match on type
        if (filteredNodes.includes(nodeType)) {
            console.log(`Filtering ${nodeType} - direct match on full type`);
            return true;
        }
        
        // 2. Direct match on name
        if (filteredNodes.includes(nodeName)) {
            console.log(`Filtering ${nodeType} - match on node name`);
            return true;
        }
        
        // 3. Match on simple name (what appears in the palette)
        if (filteredNodes.includes(simpleName)) {
            console.log(`Filtering ${nodeType} - match on simple name: ${simpleName}`);
            return true;
        }
        
        // 4. Match on the last part of the node type path
        const lastTypeSegment = nodeType.split(/[\/:]/).pop();
        if (filteredNodes.includes(lastTypeSegment)) {
            console.log(`Filtering ${nodeType} - match on type segment: ${lastTypeSegment}`);
            return true;
        }
        
        // 5. Match on the last part of the node name path
        const lastNameSegment = nodeName.split(/[\/:]/).pop();
        if (filteredNodes.includes(lastNameSegment)) {
            console.log(`Filtering ${nodeType} - match on name segment: ${lastNameSegment}`);
            return true;
        }
        
        // 6. Case insensitive partial match
        for (const filter of filteredNodes) {
            const lowerFilter = filter.toLowerCase();
            if (nodeType.toLowerCase().includes(lowerFilter) ||
                nodeName.toLowerCase().includes(lowerFilter) ||
                (simpleName && simpleName.toLowerCase().includes(lowerFilter))) {
                console.log(`Filtering ${nodeType} - partial match with: ${filter}`);
                return true;
            }
        }
        
        // Not filtering this node
        console.log(`Not filtering ${nodeType}`);
        return false;
    }

    function addNodeType(nt, def) {
        // Simplest possible filtering - check exact type and category first
        if (filteredNodes.includes(nt) || 
            (def.category && filteredCategories.includes(def.category))) {
            console.log("FILTERED OUT NODE:", nt);
            return;
        }
        
        // Extract node name for comparison
        var nodeName = nt.split("/").pop().split(":").pop();
        
        // Check if this node type should be filtered
        if (filteredNodes.includes(nodeName)) {
            console.log("FILTERED OUT NODE BY NAME:", nt, "matched", nodeName);
            return;
        }
        
        // Also check for partial matches
        for (var i = 0; i < filteredNodes.length; i++) {
            if (nt.indexOf(filteredNodes[i]) >= 0 || 
                (def.paletteLabel && def.paletteLabel.indexOf(filteredNodes[i]) >= 0)) {
                console.log("FILTERED OUT NODE BY PARTIAL MATCH:", nt, "matched", filteredNodes[i]);
                return;
            }
        }
        
        // Advanced Node-RED naming: module/name or ns:name format
        var nodeNameParts = nt.split(":");
        var nodeName = nodeNameParts.length === 2 ? nodeNameParts[1] : nt;
        ...
        if (exclusion.indexOf(nodeCategory)===-1) {
            // Continue with existing code to add the node
            ...
            // Skip category creation entirely
            // createCategory(nodeCategory,rootCategory,rootCategory,(coreCategories.indexOf(rootCategory) !== -1)?"node-red":def.set.id);

            // Add directly to main palette container
            $("#red-ui-palette-container").append(d);
    ...
    function removeNodeType(nt) {
        console.log("Removing node type from palette:", nt);
        var paletteNode = getPaletteNode(nt);
        var categoryNode = paletteNode.closest(".red-ui-palette-category");
        paletteNode.remove();
        
        // If this is a filtered node, add it to our tracking array if not already there
        var nodeName = nt.split("/").pop().split(":").pop();
        if (!filteredNodes.includes(nt) && !filteredNodes.includes(nodeName)) {
            console.log("Adding removed node to filtered list:", nt);
            filteredNodes.push(nt);
        }
    ...
    function refreshFilter() {
        ...
        // We no longer need to manage categories since we're using a flat layout
    }
    ...
    function init() {
        ...
        // Remove existing filtered nodes from the registry before they get added to palette
        function removeFilteredNodesFromRegistry() {
            console.log("Removing filtered nodes from registry");
            var allNodeTypes = RED.nodes.registry.getNodeTypes();
            
            allNodeTypes.forEach(function(nodeType) {
                var nodeName = nodeType.split("/").pop().split(":").pop();
                if (filteredNodes.includes(nodeType) || filteredNodes.includes(nodeName)) {
                    console.log("Removing node from registry:", nodeType);
                    RED.nodes.registry.removeNodeType(nodeType);
                }
            });
        }
        
        // Call immediately and set up a timeout to ensure it runs after all nodes are registered
        removeFilteredNodesFromRegistry();
        setTimeout(removeFilteredNodesFromRegistry, 2000);
        
        // Dump all node types for debugging
        console.log("=== ALL NODE TYPES ===");
        var allNodeTypes = RED.nodes.registry.getNodeTypes();
        console.log("NodeTypes:", allNodeTypes);
        
        // Get more details about each node type
        console.log("=== NODE TYPE DETAILS ===");
        allNodeTypes.forEach(function(nt) {
            var def = RED.nodes.getType(nt);
            console.log("Node Type:", nt);
            console.log("  Category:", def.category);
            console.log("  Name:", def.paletteLabel || def.label || nt);
        });
        console.log("=== END NODE TYPES ===");
        
        // Validate filtered nodes list
        console.log("=== FILTERED NODES LIST ===");
        console.log(filteredNodes);
        console.log("=== END FILTERED NODES LIST ===");
        
        // Load filtered nodes from settings
        if (RED.settings.theme('palette.filter.filteredNodes')) {
            console.log("Loading filtered nodes:", RED.settings.theme('palette.filter.filteredNodes'));
            filteredNodes = RED.settings.theme('palette.filter.filteredNodes');
        }
        
        // Load filtered categories from settings
        if (RED.settings.theme('palette.filter.filteredCategories')) {
            console.log("Loading filtered categories:", RED.settings.theme('palette.filter.filteredCategories'));
            filteredCategories = RED.settings.theme('palette.filter.filteredCategories');
        }
