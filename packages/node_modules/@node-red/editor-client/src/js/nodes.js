...
RED.nodes = (function() {
    ...
    function convertNode(n, opts) {
        var node = {
            ...
            name: n.name,
            z: n.z,
            // Explicitly add x and y if they exist
            ...(n.x !== undefined && { x: n.x }),
            ...(n.y !== undefined && { y: n.y }),
            wires: []
        };

        if (n._alias) { node._alias = n._alias; }
        if (n._def && n._def.defaults) {
            for (var p in n._def.defaults) {
                if (n._def.defaults.hasOwnProperty(p) && n[p] !== undefined) {
                    node[p] = n[p];
                }
            }
        }
        ...
        if (n.wires) {
            for (var i=0;i<n.wires.length;i++) {
                if (n.wires[i]) {
                    var wire = [];
                    for (var j=0;j<n.wires[i].length;j++) {
                        var target = n.wires[i][j];
                        var conditions = null;
                        if (n._linkConditions && n._linkConditions[i] && n._linkConditions[i][target]) {
                            conditions = n._linkConditions[i][target];
                        }
                        // Check if the target is already an object with a target property (double nesting)
                        if (typeof target === 'object' && target !== null && target.target) {
                            console.log("Found nested target object, correcting format:", target);
                            // Use the inner target directly
                            wire.push({
        ...
        if (n._def && n._def.inputs) {
            node.inputs = n._def.inputs;
        }
        if (n._def && n._def.outputs) {
            node.outputs = n._def.outputs;
        }

        if (opts && opts.includeCredentials) {
            if (n.credentials) {
    ...
    // --- Add this function before createCompleteNodeSet ---
    function rebuildWiresFromLinks() {
        console.log("[rebuildWiresFromLinks] Rebuilding wires...");
        // Clear wires for all nodes
        RED.nodes.eachNode(function(node) {
            node.wires = [];
        ...
        // Rebuild wires from links array
        for (let i = 0; i < links.length; i++) {
            const link = links[i];
            if (!link.source || !link.target) continue;
            if (typeof link.sourcePort !== 'number') continue;
            
            // Create the wire object with the correct format - directly include target ID
            let wireObj = { target: link.target.id };
            
            // If there are conditions, add them directly to this object
            if (link._condition) {
                wireObj.conditions = link._condition;
            } else if (link.source._linkConditions && link.source._linkConditions[link.sourcePort] && 
                      link.source._linkConditions[link.sourcePort][link.target.id]) {
                wireObj.conditions = link.source._linkConditions[link.sourcePort][link.target.id];
            }
            
            // Ensure the source node has a wires array for this port
            if (!link.source.wires[link.sourcePort]) {
                link.source.wires[link.sourcePort] = [];
            }
            
            // Add the wire object to the source node's wires
            link.source.wires[link.sourcePort].push(wireObj);
            
            // Debug log to verify format
            if (wireObj.conditions) {
                console.log(`[rebuildWiresFromLinks] Created wire from ${link.source.id}:${link.sourcePort} to ${link.target.id} with conditions:`, wireObj);
            }
        }
    }

    // --- In createCompleteNodeSet, call this at the start ---
    function createCompleteNodeSet(opts) {
        rebuildWiresFromLinks();
        var exportNodes = [];
        var exportSubflows = [];
        var exportConfigNodes = [];
        var exportWorkspaces = [];
        var exportGroups = [];
        var exportJunctions = [];

        for (var i=0;i<workspacesOrder.length;i++) {
            if (workspaces[workspacesOrder[i]].type == "tab") {
                exportWorkspaces.push(convertWorkspace(workspaces[workspacesOrder[i]], opts));
            }
        }
        for (var id in subflows) {
            if (subflows.hasOwnProperty(id)) {
                exportSubflows.push(convertSubflow(subflows[id], opts));
            }
        }
        // Use convertNode for groups, junctions, configNodes, and regular nodes
        for (var id in groups) {
            if (groups.hasOwnProperty(id)) {
                exportGroups.push(convertNode(groups[id], opts)); // Use convertNode
            }
        }
        for (var id in junctions) {
            if (junctions.hasOwnProperty(id)) {
                exportJunctions.push(convertNode(junctions[id], opts)); // Use convertNode
            }
        }
        for (var id in configNodes) {
            if (configNodes.hasOwnProperty(id)) {
                exportConfigNodes.push(convertNode(configNodes[id], opts)); // Use convertNode
            }
        }
        RED.nodes.eachNode(function(n) {
            exportNodes.push(convertNode(n, opts)); // Use convertNode instead of internal helper
        })
        return {
            nodes: exportNodes,
            subflows: exportSubflows,
            configNodes: exportConfigNodes,
            workspaces: exportWorkspaces,
            groups: exportGroups,
            junctions: exportJunctions
        };
    }
    ...
    function importNodes(newNodesObj,options) {
        ...
        var importedLinks = [];
        var importedGroups = [];
        var importedJunctions = [];
        var importedWorkspaces = [];
        var importedSubflows = [];

        function importNode(n) {
            console.log("importNode========>",n);
            var node = {
                id: n.id,
                type: n.type,
                name: n.name,
            ...
            if (n.wires) {
                node.wires = [];
                node._linkConditions = {};
                for (var i=0;i<n.wires.length;i++) {
                    if (n.wires[i]) {
                        var wire = [];
                        node._linkConditions[i] = {};
                        for (var j=0;j<n.wires[i].length;j++) {
                            var targetObj = n.wires[i][j];
                            var target;
                            
                            
                            // Handle different wire formats
                            if (typeof targetObj === 'string') {
                                // Simple string ID
                                target = targetObj;
                            } else if (typeof targetObj === 'object' && targetObj !== null) {
                                if (targetObj.target) {
                                    if (typeof targetObj.target === 'object' && targetObj.target.target) {
                                        // Double-nested format - extract the ID and conditions
                                        
                                        target = targetObj.target.target;
                                        if (targetObj.target.conditions) {
                                            node._linkConditions[i][target] = targetObj.target.conditions;
                                        }
                                    } else {
                                        // Standard format with target ID and conditions
                                        target = targetObj.target;
                                        if (targetObj.conditions) {
                                            node._linkConditions[i][target] = targetObj.conditions;
                                        }
                                    }
                                } else {
                                    // Unknown object format, try to use as is
                                    console.warn("[importNode] Unknown wire format:", targetObj);
                                    target = targetObj;
                                }
                            } else {
                                // Fallback
                            ...
                            // Add the target ID to the wire array
                            wire.push(target);
                        }
                        node.wires.push(wire);
                    } else {
            ...
            if (options && options.includeCredentials && n.credentials) {
                node.credentials = n.credentials;
            }
        ...
        //     console.log("nodes check 1 ========>",n);
        //     console.log("def check 1 ========>",def);
        //     if (def && def.category == "config") {
        //         console.log("config node",n,def);
        ...
        // main processing of nodes : main check **********
        for (i=0;i<newNodes.length;i++) {
            n = newNodes[i];
            // TODO: remove workspace in next release+1
        ...
        for (i=0;i<new_nodes.length+new_junctions.length;i++) {
            ...
            if (n.wires) { 
                console.log("n.wires check 4 ========>",n.wires);
                // Remove reliance on potentially undefined n._linkConditions here
                // const originalLinkConditions = n._linkConditions; 
                for (var w1=0;w1<n.wires.length;w1++) {
                    var wiresArray = (Array.isArray(n.wires[w1]))?n.wires[w1]:[n.wires[w1]]; // Renamed for clarity
                    console.log("wires check 5 ========>",wiresArray);
                    for (var w2=0;w2<wiresArray.length;w2++) {
                        const wireObject = wiresArray[w2]; // This is the object { target: "...", conditions: {...} }
                        console.log("wires check 6 (wireObject) ========>", wireObject);

                        // Ensure wireObject is actually an object with a target
                        if (typeof wireObject !== 'object' || wireObject === null || !wireObject.target) {
                            console.warn("Skipping invalid wire object:", wireObject);
                            continue; 
                        }

                        const targetId = wireObject.target; // Correctly get targetId
                        console.log("wires check 6 (targetId) ========>", targetId);

                        if (node_map.hasOwnProperty(targetId)) {
                            if (n.z === node_map[targetId].z) {
                                // Create base link object
                                var link = {source:n, sourcePort:w1, target:node_map[targetId]};
                                
                                // Directly use conditions from the wireObject if they exist
                                if (wireObject.conditions) {
                                    link._condition = wireObject.conditions;
                                    console.log("Attaching conditions to link:", link._condition); // DEBUG LOG
                                }
                                
                                // Add the potentially conditioned link
                                addLink(link);
                                new_links.push(link);
                            } else {
                                console.log("Warning: dropping link that crosses tabs:",n.id,"->",targetId);
                            }
                        } else {
                             console.log("Warning: Target node not found in map:", targetId);
                        }
                    }
                }
                // Keep n.wires for now, let rebuildWiresFromLinks handle final state
                // delete n.wires; 
            }
            if (n.g && node_map[n.g]) {
                n.g = node_map[n.g].id;
        ...
        // Load link conditions from imported nodes
        var existingConditions = RED.settings.get('edgeConditions') || {};
        var conditionsUpdated = false;
        new_nodes?.forEach(function(n) { // Iterate over the processed new_nodes array
            if (n._linkConditions) {
                // console.log("Found _linkConditions on node:", n.id, n._linkConditions);
                for (const key in n._linkConditions) {
                    if (Object.hasOwnProperty.call(n._linkConditions, key)) {
                        const condition = n._linkConditions[key];
                        const fullLinkId = n.id + ":" + key; // Reconstruct full ID (sourceId:sourcePort:targetId)
                        existingConditions[fullLinkId] = condition;
                        conditionsUpdated = true;
                        // console.log("  Storing condition for:", fullLinkId);
                    }
                }
                delete n._linkConditions; // Remove property after processing
            }
        });
        if (conditionsUpdated) {
             RED.settings.set('edgeConditions', existingConditions);
             // We might need contextMenu to reload/reapply these conditions
             // This usually happens automatically on load/refresh, but call if needed.
             // if (RED.contextMenu && typeof RED.contextMenu.loadEdgeConditionsFromFlow === 'function') {
             //      RED.contextMenu.loadEdgeConditionsFromFlow(); 
             // }
        }

        if (recoveryWorkspace) {
            RED.notify(RED._("clipboard.recoveredNodesMessage", {count: nodeZmap[recoveryWorkspace.id].length}), "warning", true, 5000);
        }
