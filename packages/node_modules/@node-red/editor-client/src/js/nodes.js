...
RED.nodes = (function() {
    ...
    function importNodes(newNodesObj,options) { // createNewIds,createMissingWorkspace) {
                ...
                for (var i=0;i<n.wires.length;i++) {
                    if (n.wires[i]) {
                        ...
                        for (var j=0;j<n.wires[i].length;j++) {
                            ...
                            // Log for debugging
                            console.log(`[importNode] Processing wire ${i}:${j}:`, JSON.stringify(targetObj));
                            ...
                            if (typeof targetObj === 'string') {
                                ...
                                if (targetObj.target) {
                                    if (typeof targetObj.target === 'object' && targetObj.target.target) {
                                        ...
                                        console.log("[importNode] Found double-nested target:", targetObj.target);
        ...
        for (i=0;i<new_nodes.length+new_junctions.length;i++) {
            ...
            if (n.wires) { 
                const originalLinkConditions = n._linkConditions; // Store conditions parsed earlier
                for (var w1=0;w1<n.wires.length;w1++) {
                    var wires = (Array.isArray(n.wires[w1]))?n.wires[w1]:[n.wires[w1]];
                    for (var w2=0;w2<wires.length;w2++) {
                        const targetId = wires[w2]; // This is the target ID string
                        if (node_map.hasOwnProperty(targetId)) {
                            if (n.z === node_map[targetId].z) {
                                ...
                                // Check if conditions exist for this wire and add them
                                if (originalLinkConditions && originalLinkConditions[w1] && originalLinkConditions[w1][targetId]) {
                                    link._condition = originalLinkConditions[w1][targetId];
                                }
                ...
                delete n.wires; // Wires can still be deleted after processing
                // Optional: delete n._linkConditions; // Clean up temporary property?
            }
            if (n.g && node_map[n.g]) {
                n.g = node_map[n.g].id;
            } else {
