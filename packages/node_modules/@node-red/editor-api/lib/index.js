...
function init(settings,_server,storage,runtimeAPI) {
    ...
    if (settings.httpAdminRoot !== false) {
        ...
        // Add MQTT Configuration endpoints back
        adminApp.get("/settings/global-mqtt", function(req, res) {
            const fs = require('fs'); // Ensure fs is required
            const path = require('path'); // Ensure path is required
            const configPath = path.join(settings.userDir, 'mqttConfig.json');
            try {
                let config = { broker: "localhost", port: "1883", qos: "0" }; // Default
                if (fs.existsSync(configPath)) {
                    try {
                        config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
                    } catch (parseErr) {
                         console.error(`(API) Error parsing ${configPath}: ${parseErr.toString()}. Using defaults.`);
                         // Keep default config
                    }
                } else {
                     console.log(`(API) ${configPath} not found. Returning defaults.`);
                     // Keep default config
                }
                res.json(config);
            } catch (err) {
                console.error("(API) Error getting MQTT config:", err);
        ...
        adminApp.post("/settings/global-mqtt", function(req, res) {
            const configPath = path.join(settings.userDir, 'mqttConfig.json');
            // runtimeAPI should be accessible from the outer init function's scope
            try {
                ...
                // write config to mqttConfig.json file using absolute path
                fs.writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf8');
                
                // Emit an event to notify nodes that the config file has been updated
                
                runtimeAPI.events.emit("mqtt-config-file-updated", config); // Pass the new config
