...
function init(settings,_server,storage,runtimeAPI) {
    ...
    if (settings.httpAdminRoot !== false) {
        ...
        adminApp.get("/api/load_triplets", function(req, res) {
            console.log("Triplets API endpoint called");
            
            try {
                // Define the path to the data file
                var dataFilePath = path.join(settings.userDir || process.cwd(), 'public', 'data', 'triplets_data.json');
                console.log("Looking for triplets data at:", dataFilePath);
                
                // Check if the file exists
                if (!fs.existsSync(dataFilePath)) {
                    console.log("Triplets data file not found");
                ...
                var tripletsData = JSON.parse(dataFileContents);
                
                var response = {
                    success: true,
                    timestamp: new Date().toISOString(),
                    ...tripletsData,
                    count: (tripletsData.entities ? tripletsData.entities.length : 0) + 
                           (tripletsData.relations ? tripletsData.relations.length : 0)
                };
                
                console.log("Returning triplets data with entities:", (tripletsData.entities || []).length, 
                          "and relations:", (tripletsData.relations || []).length);
                          
                res.json(response);
                
            } catch(err) {
                // Handle any errors
                console.error("Error in /api/load_triplets endpoint:", err);
                res.status(500).json({
                    success: false,
                    error: err.toString()
                });
        ...
        adminApp.get("/api/load_doxigen", function(req, res) {
            ...
            try {
                ...
                console.log("Returning doxigen data with entities:", (doxigenData.entities || []).length, 
                          "and relations:", (doxigenData.relations || []).length);
